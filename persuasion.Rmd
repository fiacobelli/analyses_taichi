...
pre.good<-pre.good[pre.good$sessionID%in%pre.good.sid,]
post<-post[post$sessionID %in% pre.good$sessionID,]
#View(pre.good)
```
...
```{r}
qplot(x=c("Pre","Post"),weight=c(interest.pre,interest.post),xlab="Survey", ylab="Score") + scale_x_discrete(limits=c("Pre","Post"))
```
And a t-test:
```{r}
...
```{r}
qplot(data=post, x=guru_understanding, binwidth=1, xlab="1=Strongly Disagree, 5=Totally agree",main="HG Understood me", ylab="Frequency")
# and a summary
summary(post$guru_understanding)
```
...
```{r}
t.pos<-table(pre.good$position)
t.gender<-table(pre.good$gender)
t.eth<-table(pre.good$ethnicity)
t.age<-table(pre.good$age_range)
names(t.pos)<-c("Ugrad","Grad","Faculty","Staff")
names(t.gender)<-c("M","F","Other")
names(t.eth)<-c("Cau","Hisp","ME","Pac","AA","other")
names(t.age)<-c("18-21","22-25","26-30","31-40","41-50")
t.pos
t.gender
t.eth
t.age
...
Now, in percentages:
```{r}
t.pos/length(pre.good$position)
t.gender/length(pre.good$gender)
t.eth/length(pre.good$ethnicity)
t.age/length(pre.good$age_range)
```
...
cols.names <- c("Pre","Post")
t.test(pre.interest,post.interest,paired=TRUE)
qplot(x=cols.names,weight=c(mean(pre.interest),mean(post.interest)),xlab="Survey", ylab="Score", fill=cols.names) + scale_x_discrete(limits=cols.names)+
...
raw_merged$tai_chi_interest_delta<-raw_merged$taichi_learn-raw_merged$taichi_interest
...
t.rows <- c("Decreased", "Remained Same", "Increased", "Total")
t.cols <- c("No","Yes","Total")

t.data.1 <- c(nprinted.decreased.interest, nprinted.same.interest, nprinted.increased.interest, nprinted.decreased.interest + nprinted.same.interest + nprinted.increased.interest)

t.data.2<- c(printed.decreased.interest, printed.same.interest, printed.increased.interest, printed.decreased.interest + printed.same.interest + printed.increased.interest)

t.data.3 <- c ( printed.decreased.interest + nprinted.decreased.interest, printed.same.interest + nprinted.same.interest, printed.increased.interest+nprinted.increased.interest, length(pre.good$sessionID))

m <- cbind(t.data.1,t.data.2)
m <- cbind(m,t.data.3)
colnames(m) <- t.cols
rownames(m) <- t.rows

knitr::kable(as.data.frame(m),row.names=T)
```
...
```{r}
a<-length(pint.merged$sessionID)
b<-length(printed.pre$sessionID)
a/b*100
```
